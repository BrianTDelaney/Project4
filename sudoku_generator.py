...
class SudokuGenerator:
  ...
  def __init__(self, row_length, removed_cells): # Constructor for the SudokuGenerator class.
    ...
    self.box_length = int(math.sqrt(int(row_length))) # the length of each box is set as the square root of row_length
  ...
  def valid_in_col(self, col, num): # Determines if num is contained in the given column(vertical) of the board
    for i in range(self.row_length): # this iterates through each row in the column
      if self.board[i][col] == num: # this checks if the value in the box is equal to the specified number
  ...
  def valid_in_box(self, row_start, col_start, num): # Determines if num is contained in the 3x3 box from specified on the board
    for i in range(row_start, row_start + 3): # this iterates through the each row and column in the list
      for j in range(col_start, col_start + 3): # this iterates through each 3x3 box in the list
        if self.board[i][j] == num: # this checks to see if the value in the box is equal to a specified number
  ...
  def fill_box(self, row_start, col_start): # Fills the specified 3x3 box with values by generating a rando digit which has not been used in the box
    for i in range(row_start, row_start + 3): # this iterates through the entire 2D list
      for j in range(col_start, col_start + 3): # this iterates through each 3x3 box in the list
